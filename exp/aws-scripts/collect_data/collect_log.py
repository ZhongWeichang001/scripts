...
def draw(startTime, latency, line_color, output_file_suffix, hidden_y=False, x_start=400, x_end=800, y_start=1, y_end=100000, fig_width=8, fig_height=6, x_interval=100, p99_data=None, min_data=None, rec_1=None, rec_2=None, rec_3=None, rec_4=None):
...
def plot(collected_data, output_file_suffix='', x_start=0, x_end=1000, x_interval=100):
    ...
    draw(time, latency, green_color, output_file_suffix, hidden_y=False, y_start=1, y_end=100000, x_start=x_start, x_end=x_end, fig_height=2, fig_width=5, x_interval=x_interval)
...
if __name__ == "__main__":
    ...
    output_file_paths = [
        '%s/flink-ubuntu-taskexecutor-0-ip-172-31-31-236.out' % dir_path,
    ]
    ...
    if len(collected_data) > 0:
        ...
        if len(sys.argv) >= 5:
            plot(collected_data, output_file_suffix=sys.argv[1], x_start=int(sys.argv[2]), x_end=int(sys.argv[3]), x_interval=int(sys.argv[4]))
        else:
            plot(collected_data)
    else:
